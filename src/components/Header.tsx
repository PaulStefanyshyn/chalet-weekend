  ...
  return (
    <header className="absolute top-0 left-0 w-full z-50 bg-transparent">
      <div className="max-w-7xl mx-auto px-6 py-8 flex justify-between items-center relative">
        ...
        <div className={`fixed top-0 right-0 h-screen w-70 bg-chalet-dark z-50 transform transition-transform duration-300 ease-in-out flex flex-col p-8 overflow-y-auto ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex justify-between items-center border-b border-white/10 pb-6 mb-6">
            <span className="text-chalet-input font-sans font-normal text-[18px] drop-shadow-md">Навігація</span>
            <button onClick={() => setIsOpen(false)} className="text-chalet-input">
              <X size={28} />
            </button>
          </div>
